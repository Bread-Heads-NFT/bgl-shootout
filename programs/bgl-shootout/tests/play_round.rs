...
use bgl_shootout::{
    instruction::{EndGameArgs, PlayRoundArgs, StartGameArgs},
    state::{Action, GameAccount, GAME},
};
...
async fn play_round() {
    ...
    assert_eq!(game_account.num_rounds, 3);

    for round in 0..3 {
        let play_args = PlayRoundArgs {
            match_name: name.clone(),
            mint,
            action: Action::Shoot,
        };

        let ix = bgl_shootout::instruction::play_round(
            &game_pda_address.0,
            &context.payer.pubkey(),
            play_args,
        );
        ...
        context.warp_to_slot((round + 1) * 10).unwrap();
    ...
    let end_args = EndGameArgs {
        match_name: name,
        mint,
    };

    let ix =
        bgl_shootout::instruction::end_game(&game_pda_address.0, &context.payer.pubkey(), end_args);
